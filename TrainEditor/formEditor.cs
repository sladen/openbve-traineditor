...
namespace TrainEditor {
	public partial class formEditor : Form {
		...
		private bool SaveControlContent() {
			...
			// cab
			if (!SaveControlContent(textboxX, "X", tabpagePropertiesOne, NumberRange.Any, out Train.Cab.X)) return false;
			if (!SaveControlContent(textboxY, "Y", tabpagePropertiesOne, NumberRange.Any, out Train.Cab.Y)) return false;
			if (!SaveControlContent(textboxZ, "Z", tabpagePropertiesOne, NumberRange.Any, out Train.Cab.Z)) return false;
			// car
		...
		private void LinkDocumentationLinkClicked(object sender, LinkLabelLinkClickedEventArgs e) {
			const string Url = "http://openbve.trainsimcentral.co.uk/develop/train_train_dat.html";
			try {
